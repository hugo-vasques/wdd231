/* ----- Global Logic (Header and Footer) ----- */
...
if (searchBar) {
    ...
    function getPlatformIcons(platformString) {
        ...
        if (lowerStr.includes('pc') || lowerStr.includes('windows') || lowerStr.includes('mac')) iconsHTML += icons.pc;
        if (lowerStr.includes('mobile') || lowerStr.includes('android') || lowerStr.includes('ios')) iconsHTML += icons.mobile;
        if (lowerStr.includes('playstation') || lowerStr.includes('ps4') || lowerStr.includes('ps5')) iconsHTML += icons.playstation;
        if (lowerStr.includes('xbox')) iconsHTML += icons.xbox;
        if (lowerStr.includes('nintendo') || lowerStr.includes('switch') || lowerStr.includes('wii') || lowerStr.includes('64')) iconsHTML += icons.nintendo;
        if (iconsHTML === `<span class="platform-icons-container">`) iconsHTML += icons.generic;
        iconsHTML += `</span>`;
        return iconsHTML;
    }
...
if (filterButtons.length > 0 && coursesContainer) {
    ...
    const courses = [
        ...
        { code: 'WDD231', title: 'Web Frontend Dev II', credits: 2, category: 'WDD', completed: false }, // Actual
    ];

    function createCourseTag(course) {
        const tag = document.createElement('span');
        tag.classList.add('skill-tag');
        if (course.completed) tag.classList.add('completed');
        else tag.classList.add('current');
        tag.textContent = `${course.code}: ${course.title}`;
        tag.setAttribute('data-category', course.category);
        return tag;
    }

    function renderCourses(list) {
        coursesContainer.innerHTML = '';
        list.forEach(c => coursesContainer.appendChild(createCourseTag(c)));

        const total = list.reduce((sum, c) => sum + (c.completed ? c.credits : 0), 0);
        const creditsElement = document.getElementById('totalCredits');
        if (creditsElement) creditsElement.textContent = total;
    }
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            const filteredList = filterCourses(filterValue);
